...
- The renderer draws in two passes. Geometry accumulates into an offscreen texture that is only partially erased each frame (`VisualizerMode.trailFade`), then a second pass tone-maps that texture onto the drawable. The accumulation buffer is what makes trails possible — the swap chain rotates through several drawables, so the previous frame does not survive in the drawable itself. Two struct layouts must stay in step with the shader: `VisualizerVertex` against `VIn` (float2 at 0, float4 at 16, float at 32, 48-byte stride) and `VisualizerUniforms` against `Uniforms` (five floats then an int at offset 20, 24-byte stride).
