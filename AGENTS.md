...
- `AudioEqualizer/Models/DeviceProfile.swift` and `AudioEqualizer/ViewModels/DeviceProfileStore.swift`: per-output-device EQ profiles, persisted to `deviceProfiles.json` beside the presets. Keyed by Core Audio device UID, never by name — names are not unique and change, the UID survives reconnects. The stored name exists only so absent devices can still be listed.
...
- `AudioEqualizer/Audio/MilkdropPreset.swift` reads `.milk` presets: a parser and evaluator for MilkDrop's per-frame language, plus per-pixel evaluation. Three things this cost time to learn. **The static header is not the preset** — across the original pack `rot`/`dx`/`dy`/`cx`/`cy`/`sx`/`sy` are all at defaults, so importing header values alone yields motionless, indistinguishable presets. **The motion is per-pixel**, not per-frame: `zoom` and `rot` are assigned in `per_pixel_*`, which means the transform varies across the screen and a single global transform cannot express it — hence the warp mesh. **Built-in motion variables reset to their header values every frame**; only user variables and `q1`-`q32` persist. Without that reset, `zoom = zoom - 0.05` compounds and the preset destroys itself in seconds.
- The warp mesh is a 33x25 grid whose vertices stay put while their texture coordinates move. Evaluating the per-pixel equations at every vertex costs about 3.5 ms per frame, roughly a fifth of the 60 fps budget — measured, not assumed, before the renderer was built around it. Imported presets are faithful in *motion* only: `per_pixel` colour work, custom waves and shapes, and MilkDrop 2's HLSL blocks are all ignored.
- Preset files live in Application Support, never in the repo or the bundle: the packs are community works with unclear licensing, so they are something the user brings.
...
- Device profiles capture on a 1.2 s debounce and apply from `selectedOutputDeviceID.didSet`, which flushes any pending capture against the *outgoing* device first: the timer would otherwise fire after the switch, when `bands` already belongs to the new device, and the last edit before a switch would be lost. `isApplyingProfile` guards the capture/apply loop, since applying mutates `bands` and `masterGain`. A profile saved at a different band count applies via `rebuildEQNode()` — the same teardown/build path as `setBandCount`, never `stopIOProc`.
